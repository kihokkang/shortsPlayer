    ...
    return `
        <div class="swiper-slide" role="group">
            ...
            <div class="content-layout">
                <h1 class="short-title" style="${slideData.contentTopFont || ''}" ${!slideData.contentTop ? 'style="display: none;"' : ''}>${slideData.contentTop ? slideData.contentTop.replace(/\n/g, '<br>') : ''}</h1>
                <h1 class="short-title mid-title" style="${slideData.contentMidFont || ''}" ${!slideData.contentMid ? 'style="display: none;"' : ''}>${slideData.contentMid ? slideData.contentMid.replace(/\n/g, '<br>') : ''}</h1>
                <div class="short-content" >
                    ${slideData.reviewYn === 'Y' && slideData.reviewInfo ? `
                        <div class="hashtags" style="${slideData.contentFont || ''}">
                        ...
                        <!-- 해시태그 클릭 시 표시되는 컨텐츠 -->
                        <p class="cont-text" style="display: none; ${slideData.contentFont || ''}">
                            ${slideData.reviewInfo[0]?.content || ''}
                        </p>
                    ` : `
                        <!-- 일반 컨텐츠 표시 -->
                        <p class="cont-text" style="${slideData.contentFont || ''}">
                            ${slideData.content ? slideData.content.replace(/\n/g, '<br>') : ''}
                        </p>
                    `}
                </div>
            </div>
