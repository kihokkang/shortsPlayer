...
export const createVideoSlide = (index, videoData) => `
    <div class="swiper-slide" role="group">
        ...
        <div class="content-layout">
            <h1 class="short-title" style="${videoData.contentTopFont || ''}" ${!videoData.contentTop ? 'style="display: none;"' : ''}>${videoData.contentTop ? videoData.contentTop.replace(/\n/g, '<br>') : ''}</h1>
            <h1 class="short-title mid-title" style="${videoData.contentMidFont || ''}" ${!videoData.contentMid ? 'style="display: none;"' : ''}>${videoData.contentMid ? videoData.contentMid.replace(/\n/g, '<br>') : ''}</h1>
            <div class="short-content">
                ${videoData.reviewYn === 'Y' && videoData.reviewInfo ? `
                    <div class="hashtags" style="${videoData.contentFont || ''}">
                    ...
                    <!-- 해시태그 클릭 시 표시되는 컨텐츠 -->
                    <p class="cont-text" style="display: none; ${videoData.contentFont || ''}">
                        ${videoData.reviewInfo[0]?.content || ''}
                    </p>
                ` : `
                    <!-- 일반 컨텐츠 표시 -->
                    <p class="cont-text" style="${videoData.contentFont || ''}">
                        ${videoData.content ? videoData.content.replace(/\n/g, '<br>') : ''}
                    </p>
                `}
            </div>
        </div>
