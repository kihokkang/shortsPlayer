...
export class ShareModal {
    ...
    handleShareAction(platform) {
        ...
        // 이미지 주소 구성 (비디오일 경우 썸네일 주소, 이미지일 경우 첫번째 인덱스의 이미지 원본 주소)
        const imageUrl = currentSlide.mediaTp == "M" ? `${currentSlide.mediaList[0].serverUrl}${currentSlide.mediaList[0].thumbnailPath}` : `${currentSlide.mediaList[0].serverUrl}${currentSlide.mediaList[0].filePath}`;
        // const shareUrl = currentSlide.productUrl;
        // 테스트 데이터
        const shareUrl = "https://nain.co.kr/shop/shopdetail.html?branduid=2311410&search=&xcode=003&mcode=000&scode=&special=1&GfDT=bmp6W1w%3D";
        ...
        switch (platform) {
            case "카카오톡":
                ...
                Kakao.Link.sendDefault({
                    ...
                    content: {
                        title: title,               // 공유될 제목
                        description: "",            // 공유될 설명 (TO-DO : 상단문구 들어갈 예정)
                        imageUrl: imageUrl,         // 공유될 이미지 url
                        link: {
                            mobileWebUrl: redirectUrl, // 공유될 모바일 URL
                            webUrl: redirectUrl,       // 공유될 웹 URL
                        },
                    }
                });
                break;
